...
class SelectVideoUpload_Seven_Eight : UIViewController{
    ...
    var isClicked = true
    var previewSeven = VideoPreviewButton(frame: CGRect(x: 225,y: 274,width: 465,height: 257))
    var previewEight = VideoPreviewButton(frame: CGRect(x: 225,y: 675,width: 465,height: 257))
        ...
        Alamofire.request("http://140.122.76.201/CoInQ/v1/getvideoinfo.php", method: .post, parameters: parameters).responseJSON
            {
                ...
                if let videoinfo = JSON["videoURLtable"] as? [Any] {
                    ...
                    if !(video?.isEmpty)! {
                        ...
                        switch (existone){
                        case 1:
                            self.showthumbnail(video!, "videoseven_path", self.previewSeven,self.recSeven, self.delSeven)
                            switch (existtwo){
                            case 1:
                                self.showthumbnail(video!, "videoeight_path", self.previewEight,self.recEight, self.delEight)
                            case 2:
